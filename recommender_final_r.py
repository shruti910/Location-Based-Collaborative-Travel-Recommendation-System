...
class RecommenderFinal():
    ...
    def __init__(self):
        ...
        random.seed(6)
        with open('N:\\projectStart\\mysite\\dataset\\r_userMapping.json') as user_mapping_file, open('N:\\projectStart\\mysite\\dataset\\r_itemMapping.json') as item_mapping_file, np.load(
                'N:\\projectStart\\mysite\\dataset\\Rest_ratingMatrix.npz') as sparse_matrix:
        ...
        try:
            with np.load('r_ratingMatrixTrain.npz') as training_matrix, np.load('r_ratingMatrixTest.npz') as test_matrix:
    ...
    def split_dataset(self, training_proportion, num_ratings_cap):
        ...
        np.savez('N:\\projectStart\\mysite\\dataset\\r_ratingMatrixTrain', data=self.ratings_train.data, row=self.ratings_train.row,
                 col=self.ratings_train.col, shape=self.ratings_train.shape)
        np.savez('N:\\projectStart\\mysite\\dataset\\r_ratingMatrixTest', data=self.ratings_test.data, row=self.ratings_test.row, col=self.ratings_test.col,
    ...
    def calculate_baseline_estimates(self):
        ...
        with open('N:\\projectStart\\mysite\\pickelFiles\\rating_average.pickle', 'wb') as f3:
    ...
    def train(self):
        ...
        with open('N:\\projectStart\\mysite\\pickelFiles\\user_bias.pickle', 'wb') as f1:
            pickle.dump(self.user_biases, f1)
        with open('N:\\projectStart\\mysite\\pickelFiles\\item_bias.pickle', 'wb') as f2:
            pickle.dump(self.item_biases, f2)
        with open('N:\\projectStart\\mysite\\pickelFiles\\user_factors.pickle', 'wb') as f4:
            pickle.dump(self.user_factors, f4)
        with open('N:\\projectStart\\mysite\\pickelFiles\\item_factors.pickle', 'wb') as f5:
    ...
    def recommend(self, user_id, num_items):
        ...
        with open('N:\\projectStart\\mysite\\dataset\\r_userMapping.json') as user_mapping_file:
            self.users = json.load(user_mapping_file)
        with open('N:\\projectStart\\mysite\\dataset\\r_itemMapping.json') as item_mapping_file:
            self.items=json.load(item_mapping_file)
        #LOADING PICKLE FILES
        with open('N:\\projectStart\\mysite\\pickelFiles\\user_bias.pickle', 'rb') as f1:
            self.user_biases=pickle.load(f1)
        with open('N:\\projectStart\\mysite\\pickelFiles\\item_bias.pickle', 'rb') as f2:
            self.item_biases=pickle.load(f2)
        with np.load('N:\\projectStart\\mysite\\dataset\\Rest_ratingMatrix.npz') as sparse_matrix:
            self.ratings = sps.coo_matrix((sparse_matrix['data'], (sparse_matrix['row'], sparse_matrix['col'])),
                                          shape=sparse_matrix['shape'])
        with open('N:\\projectStart\\mysite\\pickelFiles\\rating_average.pickle', 'rb') as f3:
            self.rating_avg=pickle.load(f3)
        with open('N:\\projectStart\\mysite\\pickelFiles\\user_factors.pickle', 'rb') as f4:
            self.user_factors=pickle.load(f4)
        with open('N:\\projectStart\\mysite\\pickelFiles\\item_factors.pickle', 'rb') as f5:
        ...
        with open('N:\\projectStart\\mysite\\dataset\\restaurant.json') as restaurants_file:
