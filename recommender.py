...
class Recommender():
    ...
    def __init__(self):
        ...
        random.seed(6)
        with open('dataset/r_userMapping.json') as user_mapping_file, open('dataset/r_itemMapping.json') as item_mapping_file, np.load(
                'dataset/Rest_ratingMatrix.npz') as sparse_matrix:
    ...
    def split_dataset(self, training_proportion, num_ratings_cap):
        ...
        np.savez('dataset/r_ratingMatrixTrain', data=self.ratings_train.data, row=self.ratings_train.row,
                 col=self.ratings_train.col, shape=self.ratings_train.shape)
        np.savez('dataset/r_ratingMatrixTest', data=self.ratings_test.data, row=self.ratings_test.row, col=self.ratings_test.col,
    ...
    def train(self):
        ...
        with open('E:\\projectStart\\mysite\\pickelFiles\\user_bias.pickle', 'wb') as f1:
            pickle.dump(self.user_biases, f1)
        with open('E:\\projectStart\\mysite\\pickelFiles\\item_bias.pickle', 'wb') as f2:
    ...
    def recommend(self, user_id, num_items):
        ...
        with open('dataset/restaurant.json') as restaurants_file:
        ...
        if len(self.top_rest_names) < num_items:
            ...
            return ()
        print('yo we reached end of recommender.py fghjk*****')
