...
class RecommenderCopyH():
    ...
    def __init__(self):
        ...
        random.seed(6)
        with open('N:\\projectStart\\mysite\\dataset\\h_userMapping.json') as user_mapping_file, open('N:\\projectStart\\mysite\\dataset\\h_itemMapping.json') as item_mapping_file, np.load(
                'N:\\projectStart\\mysite\\dataset\\hotel_ratingMatrix.npz') as sparse_matrix:
            self.users = json.load(user_mapping_file)
            self.items = json.load(item_mapping_file)
            self.ratings = sps.coo_matrix((sparse_matrix['data'], (sparse_matrix['row'], sparse_matrix['col'])),
        ...
        self.user_factors = np.full((self.num_users, self.num_user_factors),
                                    self.default_factor_value)  # each row represents a P_u
        self.item_factors = np.full((self.num_items, self.num_item_factors),
                                    self.default_factor_value)  # each row represents a Q_i
        ...
        if self.use_noise:
            ...
            self.user_factors += self.default_factor_value * np.random.multivariate_normal(mean, covariance,
                                                                                           self.num_users)
            self.item_factors += self.default_factor_value * np.random.multivariate_normal(mean, covariance,
                                                                                           self.num_items)
        ...
        try:
            with np.load('h_ratingMatrixTrain.npz') as training_matrix, np.load('h_ratingMatrixTest.npz') as test_matrix:
                self.ratings_train = sps.coo_matrix(
                    (training_matrix['data'], (training_matrix['row'], training_matrix['col'])),
                    shape=training_matrix['shape'])
                self.ratings_test = sps.coo_matrix((test_matrix['data'], (test_matrix['row'], test_matrix['col'])),
    ...
    def split_dataset(self, training_proportion, num_ratings_cap):
        ...
        np.savez('N:\\projectStart\\mysite\\dataset\\h_ratingMatrixTrain', data=self.ratings_train.data, row=self.ratings_train.row,
                 col=self.ratings_train.col, shape=self.ratings_train.shape)
        np.savez('N:\\projectStart\\mysite\\dataset\\h_ratingMatrixTest', data=self.ratings_test.data, row=self.ratings_test.row, col=self.ratings_test.col,
                 shape=self.ratings_test.shape)
    ...
    def calculate_baseline_estimates(self):
        ...
        with open('N:\\projectStart\\mysite\\pickelFiles\\h_rating_average.pickle', 'wb') as f3:
            pickle.dump(self.rating_avg, f3)
        ...
        while True:
            ...
            for rating, user, item in zip(self.ratings_train.data, self.ratings_train.row, self.ratings_train.col):
                ...
                for dim in range(self.num_user_factors):
                    ...
                    # update the factors
                    self.user_factors[user, dim] += self.gamma2 * (
                            error_user_item * self.item_factors[item, dim] - self.lambda7 * self.user_factors[
                        user, dim])
                    self.item_factors[item, dim] += self.gamma2 * (
                            error_user_item * self.user_factors[user, dim] - self.lambda7 * self.item_factors[
                        item, dim])
            ...
            if iter_ctr > max_num_iters:
                break
                # SERIALISING THE ARRAYS
            print('storing pickel files')
        with open('N:\\projectStart\\mysite\\pickelFiles\\h_user_bias.pickle', 'wb') as f1:
            pickle.dump(self.user_biases, f1)
        with open('N:\\projectStart\\mysite\\pickelFiles\\h_item_bias.pickle', 'wb') as f2:
            pickle.dump(self.item_biases, f2)
        with open('N:\\projectStart\\mysite\\pickelFiles\\h_user_factors.pickle', 'wb') as f4:
            pickle.dump(self.user_factors, f4)
        with open('N:\\projectStart\\mysite\\pickelFiles\\h_item_factors.pickle', 'wb') as f5:
            pickle.dump(self.item_factors, f5)
    ...
    def recommend(self, user_id, num_items):
        ...
        with open('N:\\projectStart\\mysite\\dataset\\h_userMapping.json') as user_mapping_file:
            self.users = json.load(user_mapping_file)
        with open('N:\\projectStart\\mysite\\dataset\\h_itemMapping.json') as item_mapping_file:
            self.items=json.load(item_mapping_file)
        #LOADING PICKLE FILES
        with open('N:\\projectStart\\mysite\\pickelFiles\\h_user_bias.pickle', 'rb') as f1:
            self.user_biases=pickle.load(f1)
        with open('N:\\projectStart\\mysite\\pickelFiles\\h_item_bias.pickle', 'rb') as f2:
            self.item_biases=pickle.load(f2)
        with np.load('N:\\projectStart\\mysite\\dataset\\hotel_ratingMatrix.npz') as sparse_matrix:
            self.ratings = sps.coo_matrix((sparse_matrix['data'], (sparse_matrix['row'], sparse_matrix['col'])),
                                          shape=sparse_matrix['shape'])
        with open('N:\\projectStart\\mysite\\pickelFiles\\h_rating_average.pickle', 'rb') as f3:
            self.rating_avg=pickle.load(f3)
        with open('N:\\projectStart\\mysite\\pickelFiles\\h_user_factors.pickle', 'rb') as f4:
            self.user_factors=pickle.load(f4)
        with open('N:\\projectStart\\mysite\\pickelFiles\\h_item_factors.pickle', 'rb') as f5:
            self.item_factors=pickle.load(f5)
        ...
        top_items, self.top_predictions = zip(*predictions_sorted)
        top_items = top_items[0:num_items]
        self.top_predictions = (np.fmin(self.top_predictions, 5.0))[0:num_items]

        with open('N:\\projectStart\\mysite\\dataset\\hotel.json') as restaurants_file:
        ...
        for item in top_items:
            ...
            for item_id, item_idx in self.items.items():
                if item_idx == item:

                    self.top_rest_names.append(restaurant_data[item_id]['name'])
                    self.top_hotel_cost.append(restaurant_data[item_id]['cost'])
                    break
        print(self.top_predictions)
        print(self.top_rest_names)
        print(self.top_hotel_cost)
        if len(self.top_rest_names) < num_items:
            print('\nError: Unknown Restaurant ID.\n')
            return ()
        print('yo we reached end of recommender.py')
