...
def parse_user():
    ...
    with open('dataset/user_reformat.json') as j_file:
        user_data = json.load(j_file)

    print(len(user_data))
    print
    'userId= %s' % (user_data[0]['user_id'])
    print
    'Name= %s' % (user_data[0]['name'])
    print
    'Review Count= %s' % (user_data[0]['review_count'])
    print
    'Avg Stars= %s' % (user_data[0]['average_stars'])
...
def parse_review():
    ...
    print('categs= %s' % (review_data[0]['stars']))
...
def parse_checkIn():
    ...
    len(checkIn_data)
    print
    'Business id= %s' % (checkIn_data[0]['business_id'])
    print
    'Info= %s' % (checkIn_data[0]['checkin_info'])
...
def parse_business():
    ...
    len(business_data)
    print
    'Business id= %s' % (business_data[0]['business_id'])
    print
    'Info= %s' % (business_data[0]['name'])
    print
    'City= %s' % (business_data[0]['city'])
    print
    'State= %s' % (business_data[0]['state'])
